...
<template>
  ...
  <section class="panel table-panel"><div class="panel-head"><div><h2>文档列表</h2><p class="muted">共 {{ store.documents.length }} 个文档</p></div></div><div class="table-wrap"><table><thead><tr><th>文档</th><th>知识库</th><th>分类</th><th>版本</th><th>状态</th><th></th></tr></thead><tbody><tr v-for="doc in store.documents" :key="`${doc.knowledge_base_id}-${doc.document_id}`"><td><strong>{{ doc.title }}</strong><small>{{ doc.document_id }} · {{ doc.content.slice(0, 80) }}…</small></td><td>{{ doc.knowledge_base_id }}</td><td>{{ doc.metadata.category || '未分类' }}</td><td>v{{ doc.version }}</td><td><span class="status green">已入库</span></td><td><button class="icon-button danger-icon" title="删除文档" @click="remove(doc)"><Trash2 :size="15" /></button></td></tr></tbody></table></div></section>
</template>
